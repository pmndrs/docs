...
Embed external content like Storybook stories, YouTube videos, or any iframe-compatible content using the standard HTML `<iframe>` element.
...
By default, iframes use a 16:9 aspect ratio. You can override this with an explicit height:

```md
<iframe 
  src="https://drei.pmnd.rs/iframe.html?id=staging-accumulativeshadows--accumulative-shadow-st" 
  title="Storybook"
  height="600px"
/>
```
...
> [!TIP]
> All standard HTML iframe attributes are supported: `allow`, `sandbox`, `referrerPolicy`, etc.

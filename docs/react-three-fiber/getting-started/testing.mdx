...
When it's installed if you are using Create React App you can just add a file that ends in `.test.js` and start writing your code as React Three Test Renderer is testing library agnostic, so we hope that it works with libraries such as jest, jasmine etc.
...
With that done all thats left to do is the tree demonstration of our scene and make sure the scale prop on our mesh has updated:
...
<iframe src="https://codesandbox.io/embed/hqut4?codemirror=1&fontsize=14&hidenavigation=1&theme=dark&hidedevtools=1" />

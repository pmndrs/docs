...
<Hint>
  ...
  elements, just like you can just write &lt;div /&gt; or &lt;span /&gt; in regular React-dom. The
...
If we consult the [documentation for `BoxGeometry`](https://threejs.org/docs/#api/en/geometries/BoxGeometry) we can optionally pass three arguments for: width, length and depth:
...
There are a few shortcuts for props that have a `.set()` method (colors, vectors, ect).
...
- try different materials, like [`<meshNormalMaterial />`](https://threejs.org/docs/#api/en/materials/MeshNormalMaterial) or [`<meshBasicMaterial />`](https://threejs.org/docs/#api/en/materials/MeshBasicMaterial) and give them a color

...
If anything in the tree mutates Threejs objects instead, then React cannot be aware of it, and the display would be stale. Use the `invalidate` function to trigger frames manually. 
...
<Hint>Until React 18 comes out officially the concurrency options in Three-fiber is experimental!</Hint>

...
React 18 will introduce concurrent scheduling, specifically time slicing. This can virtualise the entire component graph, which then allows you to priotitise components. Think of how a virtual list avoid scaling issues because it only renders as many items as the screen can take. React functions very similar to this, it can potentially defer load and heavy tasks in ways that would be very hard to achieve in a plain Threejs application and thereby holds on to a stable framerate even in impossibly demanding situations.
...
The follow benchmark shows how powerful concurrency can be: https://github.com/drcmda/scheduler-test

...
You should be mindful of how many draw calls you employ. You should have no more than 1000 as the very maximum, and optimally a few hundred or less. You can win performance back by reduxing draw calls through instancing recurring objects. This way you can have hundreds of thousands of objects.
...
You use LODs to reduce quality the further an object is away. That can be a nice strategy to redux the vertex-count.

Scroll in and out to see the effect:

<Codesandbox id="12nmp" />

There is a small abstraction in drei called `Detailed` that helps you do with without the boilerplate it would normally cause you. You load or prepare a couple of stages, as many as you like, and then give them the same amount of distances from the camera starting from highest quality to lowest.
...
Nested loading means that lesser textures and models are loaded first, high-resolution later.

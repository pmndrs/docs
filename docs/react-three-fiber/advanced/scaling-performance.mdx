...
Most Threejs apps run in a game-loop, that means they render 60 times per second. Three-fiber defaults to that as well. This is perfect when your scene is always moving or has moving parts in it. But it can be demanding, this is what makes your fans spin up. It can also be wasteful because it affects the battery the most. When your scene can come to rest, animations and movement are only temporarily, then you can opt into on-demand rendering instead, which will only render when necessary.

Open your devtools for instance in the sandbox below and you will see that it is completely idle when nothing is moving. Only when you move the model around it renders.

<Codesandbox id="wvgxp" />

You activate it with the `frameloop` prop, and this is already enough if your app merely changes props, which now will request frames to be rendered automatically.
...
<Hint>
  Calling invalidate() will not render immediately, it merely requests a new frame to be rendered
  out. Calling invalidate multiple times will not render multiple times. Think of it as a flag to
  tell the system that something has changed.
</Hint>

### Triggering manual frames

One caveat is that if anything in the tree _mutates_ props, then React cannot be aware of it and the display would be stale. Use the `invalidate` function to trigger frames manually.

This is useful when something is temporarily moving, for instance an animation. If you had controls for instance it would be a good idea to invalidate in the `change` event. Drei's controls [do this automatically](https://github.com/pmndrs/drei/blob/master/src/core/OrbitControls.tsx#L40) for you. If you use react-spring to animate your scene, then it will also call `invalidate()` for you.
...
<Hint>Every asset that is loaded with useLoader is cached!</Hint>

If you access the same model or texture in different places it will always re-use them! This is especially useful if you run your GLTF assets through [gltfjsx](https://github.com/pmndrs/gltfjsx) because it links up geometries and materials and thereby creates re-usable models.
...
function Shoe(props) {
  ...
  return <mesh geometry={nodes.shoe.geometry} material={materials.canvas} {...props} />
}
...
React 18 will introduce concurrent scheduling, specifically time slicing. This will virtualise the component graph, which then allows you to priotitise components and actions. Think of how a virtual list avoids scaling issues because it only renders as many items as the screen can take, it is not affected by the amount of items it has to render, be it 10 or 100.000.000.
...
You can already use it in current Three-fiber.
...
<Hint>
  Until React 18 comes out officially the concurrency options in Three-fiber is experimental!
</Hint>

...
SetState in React is a tool for flags and reactive updates. It is not adequate for fast updates! You should never setState in loops, for instance in order to move or animate something or even if you receive fast data from somewhere: remote, events, etc. Avoid forcing a full component (+ its children) through React and its diffing mechanism 60 times per second.
...
This is safe and that is how you would do it in plain three.js as well, this is what useFrame is for!

...
Hooks allow you to tie or request specific information to your component. For instance, components that want to participate in the renderloop use can do so with `useFrame`, components that need to be informed of three.js specifics use `useThree` and so on. All hooks clean up after themselves once the component unmounts.

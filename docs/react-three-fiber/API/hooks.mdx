...
The hook is reactive, if you resize the browser for instance, you get fresh measurements, same applies to any of the state objects that may change. Reactivity does not include deeper threejs internals!
...
| viewport        | Viewport size in Threejs units                            | `{ SetState<RootState>: number, height: number, initialDpr: number, dpr: number, factor: number, distance: number, aspect: number, getCurrentViewport: (camera?: Camera, target?: THREE.Vector3, size?: Size) => Viewport }` |
...
| events          | Pointer-event handling                                    | `{ connected: TargetNode / boolean, handlers: Events, connect: (target: TargetNode) => void, disconnect: () => void }`                                                                                                       |     |

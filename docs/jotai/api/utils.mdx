...
Note that in an SSR context, the `initialValue` is replaced with the `storedValue` on mount. If the content of a component is rendered differently based on the value of an `atomWithStorage`, then a server-side rendered page will initially show the `initialValue` before being swapped to the `storedValue` during rehydration. A workaround for this is to only render the affected content client-side by wrapping it in a [custom `<ClientOnly>` wrapper](https://www.joshwcomeau.com/react/the-perils-of-rehydration/#abstractions) that only renders its contents after rehydration.
...
Note that in an SSR context, the `initialValue` is replaced with the `storedValue` on mount. If the content of a component is rendered differently based on the value of an `atomWithStorage`, then a server-side rendered page will initially show the `initialValue` before being swapped to the `storedValue` during rehydration. A workaround for this is to only render the affected content client-side by wrapping it in a [custom `<ClientOnly>` wrapper](https://www.joshwcomeau.com/react/the-perils-of-rehydration/#abstractions) that only renders its contents after rehyudration.

>>>>>>> feat(jotai): add docs for new atomWithStorage util
## atomWithReset

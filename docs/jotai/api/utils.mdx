...
### Refreshing default values

You can refresh the value of an `atomWithDefault` atom to its original default value.
...
const Counter: React.FC = () => {
  ...
  return (
    <>
      ...
      <button onClick={() => setCount2(REFRESH)}>Refresh count2 default value</button>
    </>
  )
}
```
...
Setting the value to the `REFRESH` value allows us to restore the original default value,

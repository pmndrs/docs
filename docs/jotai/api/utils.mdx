...
The atom family types will be inferred from initializeAtom.
It's generally recommended to infer types from initializeAtom.
You can explicitly declare the parameter, value, and atom's update function types using TypeScript generics.
...
Here's a typical usage with a primitive atom.

```ts
import type { PrimitiveAtom } from 'jotai'

const myFamily = atomFamily((id: number) => atom(id))
// the type of atom(id) is PrimitiveAtom<number>
// PrimitiveAtom<number> is WritableAtom<number, SetStateAction<number>>
```

If you want to explicitly declare it for a primitive atom, you need to use `SetStateAction`.

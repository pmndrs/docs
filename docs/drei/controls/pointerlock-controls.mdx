---
title: PointerLockControls
---

The implementation of this class is based on the [Pointer Lock API](https://developer.mozilla.org/en-US/docs/Web/API/Pointer_Lock_API). PointerLockControls is a perfect choice for first person 3D games.

```jsx
<PointerLockControls />
```

<StoryBookLink link="controls-pointerlockcontrols--pointer-lock-controls-scene-st" />

<StoryBookEmbed id="controls-pointerlockcontrols--pointer-lock-controls-scene-st" />

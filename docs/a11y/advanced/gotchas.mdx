...
2- Mouse is over the donut, color change is triggered and the curor pointer is displayed
...
If we display the button we can see that it's caused by the button being positionned in the middle of the donut
...
5- And as we can see, it fixes our issue. The cursor is default and no color whange while the cursor is in the hole of the donut.

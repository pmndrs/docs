...
Let's take a simple React example:
...
```jsx
<mesh onPointerDown={console.log}>
   ...
</mesh>
```
...
[See this recipe for more info about this.](/docs/recipes/raycasting)

## Render Loop

By default, Fiber will setup a simple render loop that renders the default `scene` from the default `camera` to a [`WebGLRenderer`](https://threejs.org/docs/#api/en/renderers/WebGLRenderer).
